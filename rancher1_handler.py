...
class rancher1_handler():
    """
    Handler for rancher1 REST api
    """

    # Setup default configuration values, overriden by values from os.environ later
    def_cfg = {"hostname": u"localhost",
    ...
    # default container startup configs passed to rancher
    ...
    def_containr_cfg = {u'assignServiceIpAddress': False,
                        u'createIndex': None,
                        u'created': None,
                        u'description': None,
                        u'externalId': None,
    ...
    def __init__(self, main_cfg: dict, main_logger: logging.Logger) -> None:
        global logger
        ...
        if main_cfg is not None:
            ...
            for k in os.environ.keys():
                ...
                if match:
                    self.cfg['narrenv'][match.group(1)] = os.environ[k]
                    logger.debug({"message": "Setting narrenv from environment",
                                  "key": match.group(1), "value": os.environ[k]})

    def check_session(self, userid: str) -> str:
        """
        Check to see if we already have a container for this user by trying to pull the container object
        for the userid
        """
        try:
            name = self.cfg['container_name'].format(userid)
            url = "{}/service?name={}".format(self.cfg["rancher_env_url"], name)
            r = requests.get(url, auth=(self.cfg["rancher_user"], self.cfg["rancher_password"]))
            if not r.ok:
    ...
    def start(self, session: str, userid: str, prespawn: Optional[bool] = False) -> Dict[str, str]:
        """
        ...
        """
        if prespawn is True:
            self.start_new(session, userid, True)
        else:
            prespawned = self.find_prespawned()
            ...
            if len(prespawned) > 0:
                ...
                if len(prespawned) <= self.cfg['num_prespawn']:
                    self.start_new(session, session[0:6], True)
                narr_name = self.cfg['container_name'].format(userid)
                ...
                for attempt in range(max(5, len(prespawned))):
                    ...
                    try:
                        self.rename_narrative(candidate, narr_name)
                        container = self.find_service(narr_name)
                ...
                if session:
                    ...
                    return({"session": self.start_new(session, userid, False)})
            else:
                return({"session": self.start_new(session, userid, False)})

    def start_new(self, session: str, userid: str, prespawn: Optional[bool] = False):
        """
        ...
        """
        container_config = rancher1_handler.def_containr_cfg
        if prespawn is False:
            ...
            name = self.cfg['container_name_prespawn'].format(userid)
            client_ip = None
        ...
        host_rules = " || ".join([u"Host(\"{}\")".format(hostname) for hostname in self.cfg['hostname']])
        ...
        container_config['launchConfig']['imageUuid'] = "docker:{}".format(self.cfg['image'])
        container_config['launchConfig']['environment'].update(self.cfg['narrenv'])
        container_config['name'] = name
        container_config['stackId'] = self.cfg['rancher_stack_id']
        ...
        try:
            r = requests.post(self.cfg["rancher_env_url"]+"/service", json=container_config, auth=(self.cfg["rancher_user"], self.cfg["rancher_password"]))
            logger.info({"message": "new_container", "image": self.cfg['image'], "userid": userid, "service_name": name, "session_id": session,
    ...
    def find_stack(self) -> Dict[str, str]:
        """
        ...
        """
        r = requests.get(self.cfg['rancher_meta']+"2016-07-29/self/stack/environment_name")
        env_name = r.text
        logger.info("Found environment name: {}".format(env_name))
        r = requests.get(self.cfg['rancher_meta']+"2016-07-29/self/stack/name")
        stack_name = r.text
        ...
        r = requests.get(self.cfg['rancher_url']+"projects", auth=(self.cfg['rancher_user'], self.cfg['rancher_password']))
        resp = r.json()
        ...
        r = requests.get(env_endpoint+"/stacks", auth=(self.cfg['rancher_user'], self.cfg['rancher_password']))
        resp = r.json()
    ...
    def stack_suffix(self) -> str:
        ...
        """
        return("_{}".format(self.cfg['rancher_stack_name']))

    def find_service(self, traefikname: str) -> dict:
        """
        ...
        if len(results['data']) == 0:
            ...
            if len(results['data']) > 1:
                ...
                for svc in results['data'][1:]:
                    ...
                    r = requests.delete(remove_url, auth=(self.cfg['rancher_user'], self.cfg['rancher_password']))
                    if r.ok:
    ...
    def find_stopped_services(self) -> dict:
        """
        ...
        """
        url = "{}/service?healthState=started-once".format(self.cfg['rancher_env_url'])
        r = requests.get(url, auth=(self.cfg['rancher_user'], self.cfg['rancher_password']))
        if r.ok:
            results = r.json()
    ...
    def find_image(self, name: str) -> str:
        """
        ...
        """
        try:
            container = self.find_service(name)
    ...
    def reap_narrative(self, name: str) -> None:
        res = self.find_service(name)
    ...
    def rename_narrative(self, name1: str, name2: str) -> None:
        res = self.find_service(name1)
        # if there is a None return, the image may have been reaped already just return
        if res is None:
            return
        ...
        data['description'] = 'client-ip:{} timestamp:{}'.format(client_ip,
                                                                 datetime.utcnow().strftime('%Y-%m-%dT%H:%M:%S.%fZ'))
        r = requests.put(put_url, auth=(self.cfg['rancher_user'], self.cfg['rancher_password']), data=data)
        if r.ok:
            return
        else:
            raise(Exception("Problem renaming narrative {} to {}: response code {}: {}".format(name1, name2, r.status_code, r.text)))

    def find_prespawned(self) -> List[str]:
        """ returns a list of the prespawned narratives waiting to be assigned """
        narratives = self.find_narratives()
        idle_narr = [narr for narr in narratives if self.cfg['container_name_prespawn'].format("") in narr]
        return(idle_narr)

    def find_narratives(self, image_name: Optional[str] = None) -> List[str]:
        """
        ...
        if image_name is None:
            image_name = self.cfg['image']
        query_params = {'limit': 1000}
        url = "{}/stacks/{}/services".format(self.cfg['rancher_env_url'], self.cfg['rancher_stack_id'])
        r = requests.get(url, auth=(self.cfg['rancher_user'], self.cfg['rancher_password']), params=query_params)
    ...
    def find_narrative_labels(self, svc_list: list) -> dict:
        """
    ...
    def verify_config(self, cfg2: dict) -> None:
        """
        ...
        if (self.cfg['rancher_url'] is None):
            logger.critical("rancher_url is not set, cannot operate in rancher mode")
            raise(ValueError("rancher_url configuration not set"))
        if (self.cfg['rancher_user'] is None) or (self.cfg['rancher_password'] is None):
            logger.warning("rancher_user and/or rancher_password not set")
        try:
            r = requests.get(self.cfg['rancher_url'], auth=(self.cfg['rancher_user'], self.cfg['rancher_password']))
            if (not r.ok):
            ...
            logger.critical("Error trying to connect to {}: {}".format(self.cfg['rancher_url'], str(ex)))
            raise(ex)
        if (self.cfg['rancher_stack_id'] is None or self.cfg['rancher_env_url'] is None or self.cfg['rancher_stack_name'] is None):
            logger.info("rancher_stack_id, rancher_stack_name, or rancher_env_url not set - introspecting rancher-metadata service")
            try:
                info = self.find_stack()
                self.cfg['rancher_stack_id'] = info['stack_id']
                self.cfg['rancher_stack_name'] = info['stack_name']
                self.cfg['rancher_env_url'] = info['url']
                if self.cfg['rancher_stack_id'] is None or self.cfg['rancher_env_url'] is None:
                ...
                logger.critical("Could not query rancher_meta({}) service: {}".format(self.cfg['rancher_meta'], str(ex)))
                raise(ex)
        # Make sure we can query the rancher environment endpoint
        try:
            r = requests.get(self.cfg['rancher_env_url'], auth=(self.cfg['rancher_user'], self.cfg['rancher_password']))
            if (not r.ok):
            ...
            logger.critical("Error trying to connect to {}: {}".format(self.cfg['rancher_env_url'], str(ex)))
            raise(ex)
